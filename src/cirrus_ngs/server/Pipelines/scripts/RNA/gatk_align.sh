...
log_file=$log_dir/$fastq_end1/'gatk_align.log'
exec 1>>$log_file
exec 2>>$log_file
...
if [ "$fastq_end2" == "NULL" ]
then
    $STAR --runThreadN $num_threads --genomeDir $STAR_genome --twopassMode Basic --readFilesIn $workspace/$fastq_end1$file_suffix \
    --outFileNamePrefix $workspace/$fastq_end1.

else
    $STAR --runThreadN $num_threads --genomeDir $STAR_genome --twopassMode Basic --readFilesIn $workspace/$fastq_end1$file_suffix $workspace/$fastq_end2$file_suffix \
    --outFileNamePrefix $workspace/$fastq_end1.
fi

...
awk -F '\t' '{if($0 ~ /\#/) print; else if($7 == "PASS") print}' $workspace/${fastq_end1}_filt.vcf > $workspace/${fastq_end1}_filt.vcf.pass
...
$SNPiR/convertVCF.sh $workspace/${fastq_end1}_filt.vcf.pass $workspace/${fastq_end1}_raw_variants.txt 20
# filt for total depth of at least 5 reads and 2 reads with alternate allele
awk -F '\t|,' '{if (($3 >= 5) && ($4 >= 2)) {print}}' $workspace/${fastq_end1}_raw_variants.txt > $workspace/${fastq_end1}_raw_variants.mincov.txt
...
# # alu sites
awk '{OFS="\t";$2=$2-1"\t"$2;print $0}' $workspace/${fastq_end1}_raw_variants.rmhex.txt | \
	$intersectBed \
	-wa \
	-header \
	-a stdin \
	-b $hg19_AluRegions \
	> $workspace/${fastq_end1}_alu_sites.txt 
...
awk '{OFS="\t";print $1,$2,$2}' $workspace/${fastq_end1}_alu_rnaedit_sites.txt | \
	$intersectBed \
	-wa \
	-header \
	-a $workspace/${fastq_end1}_filt.vcf.pass \
	-b stdin \
	| uniq \
	> $workspace/${fastq_end1}_alu_rnaedit_sites.vcf
...
awk '{OFS="\t";$2=$2-1"\t"$2;print $0}' $workspace/${fastq_end1}_raw_variants.rmhex.txt | \
	...
	-a stdin \
	-b $hg19_AluRegions \
	> $workspace/${fastq_end1}_raw_variants.rmhex.nonalu.txt

# filter variants in simple repeat regions
awk '{OFS="\t";$2=$2-1"\t"$2;print $0}' $workspace/${fastq_end1}_raw_variants.rmhex.nonalu.txt | \
	$intersectBed \
	-v \
	-a stdin \
	-b $hg19_RepeatMasker | \
	cut -f1,3-7 > $workspace/${fastq_end1}_raw_variants.rmhex.nonalu.rmsk.txt
...
awk '{OFS="\t";$2=$2-1"\t"$2;print $0}' $workspace/${fastq_end1}_raw_variants.rmhex.nonalu.rmsk.rmintron.rmhom.rmblat.txt | \
	$intersectBed \
	-wa \
	-a stdin \
	-b $hg19_rnaEditDB \
	> $workspace/${fastq_end1}_nonalu_rnaedit_sites.txt 

awk '{OFS="\t";print $1,$2,$2}' $workspace/${fastq_end1}_raw_variants.rmhex.nonalu.rmsk.rmintron.rmhom.rmblat.txt | \
	$intersectBed \
	-wa \
	-header \
	-a $workspace/${fastq_end1}_filt.vcf.pass \
	-b stdin \
	| uniq \
	> $workspace/${fastq_end1}_nonalu_rnaseq_vars.vcf

...
## Global variable: default configuation file for parallel Cluster
config_template_file = os.environ['HOME'] + "/.parallelcluster/config" #os.getcwd().replace("notebooks", "data") + "/config"

def install_cluster():
    """Installs aws cluster framework.

    Uses pip to install the aws cluster framework and cli.
    ...
    "pip install aws-parallelcluster" command to stdout.
    ...
    Returns:
        None
    """
    print("Installing aws-parallelcluster package...")
    print(os.popen("pip install aws-parallelcluster").read())

def upgrade_cluster():
    """Updates aws cluster framework.

    Uses pip to update awscluster framework and cli. The
    awscluster framework must already be installed on machine.
    Will not work if pip executable is not in $PATH. 
    Prints output of "pip install --upgrade aws-parallelcluster" command to
    stdout.
    ...
    Returns:
        None
    """
    print("Upgrading aws-parallelcluster package...")
    print(os.popen("pip install --upgrade aws-parallelcluster").read())

def make_config_file():
    """Creates template cluster configuration file.

    Writes to the ~/.parallelcluster/config file. If
    ...
    filewriter.write("[cluster template]" + "\n")
    filewriter.write("vpc_settings = vpc_name" + "\n")
    ...
    filewriter.write("cluster_template = template" + "\n")
...
def view_cluster_config():
    """Prints contents of cluster configuration file.

    If the file does not exist, it creates a template version
    of it in ~/.parallelcluster/config using make_config_file. The contents are
    printed to stdout.
    ...
    cluster_config = os.environ['HOME'] + "/.parallelcluster/config"

    if not os.path.isfile(cluster_config):
        if not os.path.exists(os.path.dirname(cluster_config)):
            os.makedirs(os.path.dirname(cluster_config))
        copyfile(config_template_file, cluster_config)
...
def insert_access_keys(aws_access_key_id="***",
    ...
    """Adds AWS key ID and keys to cluster configuration file.
...
def config_aws_region_name(aws_region_name="us-east-1"):
    """Adds region to cluster configuration file.
...
def config_key_name(key_name):
    """Adds private key name to cluster configuration file.
...
def config_instance_types(master_instance_type="m3.large", compute_instance_type="r3.2xlarge"):
    """Adds head node and computing node types to cluster configuration file.
...
def config_initial_cluster_size(initial_cluster_size="1"):
    """Adds starting cluster size to cluster configuration file.
...
def config_spot_price(spot_price="0.5"):
    """Adds spot price to cluster configuration file.
...
def config_s3_resource(s3_read_resource="bucket_name", s3_read_write_resource="bucket_name"):
    """Adds input and output s3 resources to cluster configuration file.
...
def config_post_install(post_install="s3://bucket_name/path/to/postinstall.sh"):
    """Adds path to post installation script to cluster configuration file.
...
def config_vpc_subnet_id(master_subnet_id="subnet-00000000",
                       vpc_id="vpc-00000000"):
    """Add master subnet ID and VPC ID to the cluster configuration file.
...
def config_ebs_snapshot_id(ebs_snapshot_id="snap-a6e477ff"):
    """Adds cirrus EBS snapshot ID to cluster configuration file
...
def config_volume_size(volume_size="200"):
    """Adds EBS volume size to the cluster configuration file.
...
def list_aws_cluster():
    """ Prints a list of current clusters.

    This is based on the the cluster cli, it must be installed
    ...
    Returns:
        None
    """
    print(os.popen("pcluster list").read())

## creating a CFNCluster with the specific cluster name
def create_aws_cluster(cluster_name="mycluster"):
    """Creates a cluster.

    The cluster is created using the cluster
    ...
    response = os.popen("pcluster create " + cluster_name).read()
...
def delete_aws_cluster(cluster_name="mycluster"):
    """Deletes a cluster.

    This function completely gets rid of a cluster.
    Prints the output of the "pcluster delete"
    call.
    ...
    Returns:
        None
    """
    print(os.popen("pcluster delete " + cluster_name).read())

if __name__ == "__main__":
    view_awscluster_config()

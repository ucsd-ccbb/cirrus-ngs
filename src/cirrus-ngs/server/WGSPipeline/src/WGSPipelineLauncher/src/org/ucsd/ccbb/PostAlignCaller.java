...
/**
 * PostAlignCaller calls PostAlignmentWorker to process realignment, variant 
 * calls and so on.
 * 
 * @author Guorong Xu
 */
public class PostAlignCaller {
	...
	/**
	 * Creates a shell command to call PostAlignmentWorker to process 
	 * realignment, variant calls and so on.
	...
	{	
		...
		{		
		    ...
		    for ( int index = 0; index < fastqList.size(); index++ ) 
		    {
				...
				for (int i = 0; i < m_chromLength.length; i ++)
				{
					...
					for (int j = 0; j < regionNum; j ++)
					{
						...
						//what's the difference between splitBAMFile and bashFileName: no difference
						 ...
						 */
						String bashFile = rootPath + "results/" + fileName + "/postalignworker_" + bashFileName + ".sh";
						...
						filewriter.write("java -Xms454m -Xmx2g -jar " + wgsmanager + " command=postalignworker S3UploadURL=" + S3UploadURL + "/" + fileName 
								+ " S3DownloadURL=" + S3UploadURL + "/" + fileName + " fileName1=" + splitBAMFile + ".sort.split.bam" 
						 ...
						 * nodes.
						 * default slot of postalignment.call is 2*/
						String command = " qsub -pe smp " + loader.get("postalignment.call.slot", "2") + " " + bashFile;
		    ...
		    new PBSTracker().trackPBSQueue(Integer.valueOf(loader.get("trackTime", "1")), "postalign");	

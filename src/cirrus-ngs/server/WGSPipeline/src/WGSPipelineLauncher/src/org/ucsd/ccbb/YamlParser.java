		...
		{	
		    ...
		    for (Object data : yaml.loadAll(input)) {  //?
                ...
                if (sample != null)
                {
                    ...
                    if (fileName.indexOf(".fq.gz") > -1)
                        m_fastqList.add(new String[]{fileName.substring(0, fileName.indexOf("_R")), S3DownloadURL, description, group});
					...
					if (group != null && !"NA".equalsIgnoreCase(group) && ! m_sampleGroups.containsKey(group))
						m_sampleGroups.put(group, fileName.substring(0, fileName.indexOf("_R")) + "/");
					else if (group != null && !"NA".equalsIgnoreCase(group) && m_sampleGroups.containsKey(group))
					{
						String value = m_sampleGroups.get(group);
						m_sampleGroups.put(group, value + fileName.substring(0, fileName.indexOf("_R")) + "/");
					}
	    		}
		    }
		    
		    System.out.println("===========================");
            /*System.out.println("m_fastqList"+m_fastqList);
            System.out.println("m_bamList"+m_bamList);
            System.out.println("===========================");*/
		} catch (IOException e)
		{
			e.printStackTrace();

...
# Count reads #
check_exit_status "$samtools view $workspace/$fastq_end1$file_suffix | sort -T $workspace/ -s -k 1,1 - \
   |  $htseq_count - $genome_gtf > $workspace/${fastq_end1}_counts.txt" $JOB_NAME $status_file
